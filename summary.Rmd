---
title: "Comparing forestry certification standards"
output: 
   html_document:
    toc: true
    code_folding: hide
...
acres$color <- gsub("FSC and PEFC \\(i.e. SFI or ATFS\\)", "PEFC (i.e. SFI or ATFS) and FSC", acres$color)
 ...
 cert <- ggplot(acres %>% filter(bar != "Total Corporate-owned timberland",
    ...
    coord_flip() + 
    labs(y = "Acres (millions)", x = "", fill = "Program") + 
...
corp <- ggplot(acres %>% filter(bar %in% c("Total Corporate-owned timberland",
    ...
    scale_y_continuous(breaks = seq(0, 150, 30), limits = c(0,155)) +
      scale_color_viridis_d(begin = .8, end = .2, option = "D") +
...
### Restrictions on clearcuts
```{r clearcuts, fig.height=3, fig.width=5}
d <- read_csv("data/clearcuts.csv")

d %>% 
  ggplot(aes(x = program, ymin= min, ymax=max, label = text, color = program)) + 
  ...
  scale_color_viridis_d(begin = .6, end = .2, option = "B") +
...
### Restrictions on harvesting near streams
```{r riparian, fig.height=6.5, fig.width=7.5}
...
fsc <- d %>% 
  ...
  coord_flip() +
  labs(x = "", y = "Feet", fill = "Minimum streamside
...
sfi <- d %>% 
  ...
  geom_col(position = "stack") + 
  geom_text( aes(y = 0), hjust = 0, size = 2.5, color = "#458B00")+
    geom_text( aes(y = 150, x = 0, label = "") )+ # to get scale to match
  coord_flip() +
  labs(x = "", y = "Feet", fill = "",
...
Safety | posted safety guidelines, contracts include safety, records kept | Written commitment to comply with OSHA
Training on OSHA |
...
### Prescriptiveness and Scope 2008-2016

<center>
```{r FSC-SFI-plot, fig.height= 10, fig.width=7.5}
...
bar <- SFIvFSC %>%
  ...
  geom_segment(data = data.frame(Program = "Activist-backed FSC-US"), aes(x = 2009.5, y = 43, xend = 2016.5, yend = 43), linetype = 1, size = .5) + 
  # geom_segment(data = data.frame(Program = "Industry-backed SFI"), aes(x = 2007.5, y = 11, xend = 2009.5, yend = 11), linetype = 1, size = .5) + 
  geom_segment(data = data.frame(Program = "Industry-backed SFI"), aes(x = 2009.5, y = 10, xend = 2016.5, yend = 10), linetype = 1, size = .5) + 
  geom_text(data = data.frame(Program = "Industry-backed SFI"), 
label = "As prescriptive or 
more on 10 issues", x = 2016, y = 12, vjust = 0, hjust = 1, size = 3) + 
  geom_text(data = data.frame(Program = "Activist-backed FSC-US"), 
label = "As prescriptive or
more on 43 issues", x = 2016, y = 28, vjust = 0, hjust = 1, size = 3) + 
...
### Prescriptiveness and Scope 2008-2015



```{r pefc-fsc-plots, fig.height= 10, fig.width=7.5}
# line plot (line)
line <- PEFCvFSC %>% 
...
label = "Increased in
18 issues 
since 2008", x = 2015, y = 16, vjust = 1, hjust = 1, size = 3) + 
  geom_text(data = data.frame(Program = "Activist-backed FSC P&C"), 
label = "Increased in 13
...
ggplot() + 
  ...
  scale_fill_grey(start = .9, end = .5) + 
    ...
    geom_segment(data = data.frame(Program = "Activist-backed FSC P&C"), aes(x = 2011.5, y = 36, xend = 2015.5, yend = 36), linetype = 1, size = .5) + 
    # geom_segment(data = data.frame(Program = "Industry-backed PEFC"), aes(x = 2007.5, y = 20, xend = 2009.5, yend = 20), linetype = 1, size = .5) + 
    geom_segment(data = data.frame(Program = "Industry-backed PEFC"), aes(x = 2009.5, y = 27, xend = 2015.5, yend = 27), linetype = 1, size = .5) + 
    geom_text(data = data.frame(Program = "Industry-backed PEFC"), 
label = "As prescriptive or 
more on 27 issues", x = 2015, y = 13, vjust = 0, hjust = 1, size = 3) + 
    geom_text(data = data.frame(Program = "Activist-backed FSC P&C"), 
label = "As prescriptive or
more on 36 issues", x = 2015, y = 20, vjust = 0, hjust = 1, size = 3) + 
    labs(title = "Relative Prescriptiveness and Scope",
       y = "Key Issues") + 
    scale_y_continuous(breaks = seq(0,48, by = 8)) + 
...
### Prescriptiveness of Policy Types (adapted from Cashore (1997))
...
| Decreasing | $\style{display: inline-block; transform: rotate(35deg)}{\searrow}$  <br> $\searrow$ | $\searrow$ <br> $\searrow$ | $\searrow$ <br> $\style{display: inline-block; transform: rotate(35deg)}{\searrow}$ |
