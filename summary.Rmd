...
```{r clearcuts, fig.height=4, fig.width=5}
d <- read_csv("data/clearcuts.csv")

fsc <- d %>% 
  filter(program %in% c("FSC")) %>% 
  ggplot(aes(x = criteria, ymin= min, ymax=max, label = text)) + 
  geom_col( aes(y = max), alpha = .2, width = .3) +
...
```{r riparian, fig.height=6.5, fig.width=8}
...
fsc <- d %>% 
  ...
  geom_text( aes(y = 0), hjust = 0, size = 3, color = "#458B00")+
  # align axis
