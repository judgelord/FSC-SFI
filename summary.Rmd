...
sfi <- d %>% 
  ...
  ggplot(aes(x = criteria, ymin= min, ymax=max, label = text, group=1)) + 
  geom_col( aes(y = max), width = .3, fill = "tan") +
    geom_segment(aes(x = "Clearcut size limit", xend = "Clearcut size limit", y = 0, yend=140), size = 1.5, color = "tan", arrow = arrow(length = unit(0.4, "cm")), lineend = "butt", linejoin= "mitre") +
      geom_segment(aes(x = "Harvest with 20% retention limit", xend = "Harvest with 20% retention limit", y = 0, yend=140), size = 1.5, color = "tan", arrow = arrow(length = unit(0.4, "cm") ), lineend = "butt", linejoin= "mitre") +
  ...
  geom_point( aes(y = min), shape = "|", size = 3) + 
  geom_point( aes(y = max), shape = "|", size = 3) + 
  geom_text( aes(y = 1), hjust = 0, size = 3) +
  geom_linerange()+
  coord_flip() + 
  scale_y_continuous(breaks = seq(0,140,20)) +
  labs(x = "", y = "Acres",
