---
title: "Project summary: comparing FSC and SFI "
output: 
   html_document:
    toc: true
    code_folding: hide
...
|:----|:----|:----|
Threatened <br> and Endangered Species | | | 
...
line <- SFIvFSCnetChange %>%
  ...
  ggplot() + 
  geom_step(aes(x = Year, y = Change, linetype = Measure) ) +  
  facet_grid(. ~ Program) + 
...
more on 10 issues", x = 2016, y = 12, vjust = 0, hjust = 1, size = 3) + 
  geom_text(data = data.frame(Program = "Activist-backed FSC-US"), 
label = "As prescriptive or
more on 42 issues", x = 2016, y = 28, vjust = 0, hjust = 1, size = 3) + 
...
line <- PEFCvFSC %>% 
  ...
  ggplot() + 
  geom_step(aes(x = Year, y = Change08, linetype = Measure)) + 
  facet_grid(. ~ Program) + 
