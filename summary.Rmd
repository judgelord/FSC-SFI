...
```{r clearcuts, fig.height=3.7, fig.width=5.5}
d <- read_csv("data/clearcuts.csv")

fsc <- d %>% 
  filter(program %in% c("FSC")) %>% 
  ggplot(aes(x = criteria, ymin= min, ymax=max, label = text, group = 1)) + 
  geom_col( aes(y = max), alpha = .2, width = .3, fill = "saddlebrown") +
    geom_line( aes(y = 10), color="white", lwd = .1)+
  geom_line( aes(y = 20), color="white", lwd = .1)+
    geom_line( aes(y = 30), color="white", lwd = .1)+
    geom_line( aes(y = 40), color="white", lwd = .1)+
    geom_line( aes(y = 50), color="white", lwd = .1)+
    geom_line( aes(y = 60), color="white", lwd = .1)+
    geom_line( aes(y = 70), color="white", lwd = .1)+
    geom_line( aes(y = 80), color="white", lwd = .1)+
    geom_line( aes(y = 90), color="white", lwd = .1)+
    geom_line( aes(y = 100), color="white", lwd = .1)+
    geom_line( aes(y = 110), color="white", lwd = .1)+
    geom_line( aes(y = 120), color="white", lwd = .1)+
  geom_point( aes(y = min),shape = 15) + 
  geom_point( aes(y = max), shape = 15) + 
  geom_text( aes(y = 1), hjust = 0, size = 3, color = "#458B00") +
  geom_linerange()+
  geom_text( aes(y = 125, x = 0, label = "") )+
  coord_flip() + 
  labs(x = "", y = "Acres",
...
sfi <- d %>% 
  ...
  geom_col(position = "stack") + 
  geom_text(aes(y = 0), hjust = 0, size = 3, color = "#458B00", check_overlap = T)+
    geom_text(aes(y = 175, x = 0), label = "" )+ # to get scale to match
  coord_flip() +
  labs(x = "", y = "", fill = "",
...
### Prescriptiveness and Scope of FSC-US and SFI 2008-2016
...
### Prescriptiveness and Scope of FSC P&C and PEFC 2008-2015
...
tile <- PEFCvFSC %>% filter(Program %in% c(
  ...
  scale_x_continuous(breaks = seq(2008,2015, by = 1)) + 
#labs(title = "Scope of Issues Addressed") + 
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        #remove legend title
        legend.title=element_blank(),
